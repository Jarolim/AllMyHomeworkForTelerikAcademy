...
var persisters = (function () {
    ...
    var nickname = localStorage.getItem("nickname"); // for controller
    var sessionKey = localStorage.getItem("sessionKey"); // for controller

    function saveUserData(userData) {   // for controller
        localStorage.setItem("nickname", userData.nickname);
        localStorage.setItem("sessionKey", userData.sessionKey);
        nickname = userData.nickname;
        sessionKey = userData.sessionKey;
    }
    function clearUserData() { // for controller for logout
    ...
    var MainPersister = Class.create({ //1
        ...
        isUserLoggedIn: function () { // for controller
    ...
    var UserPersister = Class.create({ //2
        init: function (rootUrl) {
            //...api/user/
        ...
        login: function (user, success, error) {
            ...
            httpRequester.postJSON(url, userData,
				function (data) {
				    saveUserData(data); // alert(JSON.stringify(data));
				    success(data);
				}, error);
        },
...
//var bcPersisiter = persisters.get("http://localhost:40643/api/");
...
//bcPersisiter.user.login(user, function (data) {
//    alert(JSON.stringify(data));
//}, function (err) {
//    alert(JSON.stringify(err));
//});

//bcPersisiter.game.create(game, success, error);
//bcPersisiter.message.all(message, success, error);
...
bcPersisiter.user.register(user, function (data) {
    alert(JSON.stringify(data));
}, function (err) {
    alert(JSON.stringify(err));
});