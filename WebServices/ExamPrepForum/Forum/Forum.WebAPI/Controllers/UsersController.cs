...
namespace Forum.WebAPI.Controllers
{
    public class UsersController : BaseApiController
    {
        ...
        */
            ...
            var responseMsg = this.PerformOperationAndHandleExceptions(
                () =>
                {
                    ...
                    using (context)
                    {
                        ...
                        this.ValidateNicknamme(model.Nickname);
                        this.ValidateAuthCode(model.AuthCode);

                        var usernameToLower = model.Username.ToLower();
                        var nicknameToLower = model.Nickname.ToLower();
                        var user = context.Users.FirstOrDefault(usr => usr.Username == usernameToLower || usr.Nickname == nicknameToLower);

                        if (user != null)
                        {
                            throw new InvalidOperationException("User exists yoyo");
                        }
        ...
        private void ValidateNicknamme(string nicknamme)
        {
            if (nicknamme == null)
            {
                throw new ArgumentNullException("Nickname cannot be null");
            }
            else if (nicknamme.Length < MinUsernameLength)
            {
                throw new ArgumentOutOfRangeException(
                    string.Format("Nickname must be at least {0} characters long",
                    MinUsernameLength));
            }
            else if (nicknamme.Length > MaxUsernameLength)
            {
                throw new ArgumentOutOfRangeException(
                    string.Format("Nickname must be less than {0} characters long",
                    MaxUsernameLength));
            }
            else if (nicknamme.Any(ch => !ValidNicknameCharacters.Contains(ch)))
            {
                throw new ArgumentOutOfRangeException(
