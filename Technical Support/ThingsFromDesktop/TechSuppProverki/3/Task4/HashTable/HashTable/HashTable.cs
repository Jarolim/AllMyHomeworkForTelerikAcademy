...
namespace HashTableSample
{
    public class HashTable<K, T> : IEnumerable
    {
        ...
        public void Remove(K key)
        {
            ...
            if (this.hashTable[hashedIndex].Count == 1 && !this.hashTable[hashedIndex].First.Value.Key.Equals(key))
            {
                throw new ArgumentException("Key was not found in the hash-table.");
            }
            else if (this.hashTable[hashedIndex].Count == 1 && this.hashTable[hashedIndex].First.Value.Key.Equals(key))
            {
        ...
        private void ReAddValues(LinkedList<KeyValuePair<K, T>>[] copiedArray, int newCapacity)
        {
            this.Capacity = newCapacity;
            this.hashTable = new LinkedList<KeyValuePair<K, T>>[this.Capacity];
            this.Count = 0;
