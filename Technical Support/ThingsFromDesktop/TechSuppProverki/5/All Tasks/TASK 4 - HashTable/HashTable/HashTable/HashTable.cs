...
namespace HashTableSample
{
    public class HashTable<K, T> : IEnumerable
    {
        ...
        public void Remove(K key)
        {
            ...
            if (this.hashTable[hashedIndex].Count == 1 && this.hashTable[hashedIndex].First.Value.Key.ToString() // add .Value.Key.ToString() 
                != key.ToString()) // add .ToString()
            {
                throw new ArgumentException("Key was not found in the hash-table.");
            }
            else if (this.hashTable[hashedIndex].Count == 1 && this.hashTable[hashedIndex].First.Value.Key.ToString() // add .Value.Key.ToString() 
                == key.ToString()) // add .ToString()
            {
        ...
        private void IncreaseCapacity()
        {
            ...
            ReAddValues(copiedArray, newCapacity);     // Change Capacity with newCapacity        
        }
